...
def extract_lines():
    ...
    lines_set = {
        Line(
            ...
            icon_filename=relationship.line_icon_filename,
        )
    ...
    for line in lines:
        if line.icon_url and line.icon_filename:
            ...
            with open(GENERATED_ICONS / line.icon_filename, "wb") as f:
                f.write(img_data)
            line.color = extract_dominant_color_from_svg(
                GENERATED_ICONS / line.icon_filename
            )
...
def combine_data():
    ...
    for relationship in lines_relationships:
        if relationship.mode != "METRO":
            continue
        line = next(line for line in lines if line.id == relationship.line_id)
    ...
    with open(GENERATED / "zones_metro.json", "w") as f:
        ...
        f.write(
            ta.dump_json(
                [zm for zm in zones_metro if zm.accesses and zm.lines], indent=4
            ).decode()
        )
