...
class Access(BaseModel):
    ...
    short_name: int | None = Field(
        validation_alias=AliasChoices("accshortname", "short_name")
    )
    x_lambert_93: int = Field(
        validation_alias=AliasChoices("accxespg2154", "x_lambert_93")
    )
    y_lambert_93: int = Field(
        validation_alias=AliasChoices("accyespg2154", "y_lambert_93")
    )
    geo_point: GeoPoint = Field(
        validation_alias=AliasChoices("accgeopoint", "geo_point")
    )
...
class Zone(BaseModel):
    ...
    postal_region: str = Field(
        validation_alias=AliasChoices("zdapostalregion", "postal_region")
    )
    x_lambert_93: int = Field(
        validation_alias=AliasChoices("zdaxepsg2154", "x_lambert_93")
    )
    y_lambert_93: int = Field(
        validation_alias=AliasChoices("zdayepsg2154", "y_lambert_93")
    )
...
class ZoneLineRelationship(BaseModel):
    ...
    line_icon_filename: str | None = Field(
        ...
        validation_alias=AliasChoices(
            AliasPath("picto", "filename"), "line_icon_filename"
        ),
    )
    mode: str
