...
def print_single_result(result: dict, verbose: bool = False):
    ...
    print(f"\n{result['image']}:")
    print(f"  Boxes: {result['detected_count']} detected, {result['ground_truth_count']} ground truth")
    ...
    if verbose:
        print(f"  Areas: GT={result['ground_truth_area']:,}px, Det={result['detected_area']:,}px, "
              f"Overlap={result['overlap_area']:,}px")
        if result['missed_area'] > 0:
            print(f"  Missed: {result['missed_area']:,}px ({result['missed_area']/result['ground_truth_area']:.1%} of GT)")
...
def main():
    ...
    parser.add_argument(
        "-v", "--verbose",
        action="store_true",
        help="Show detailed area information"
    )
    ...
    if args.name:
        ...
        if results:
            ...
            total_coverage = total_overlap_area / total_gt_area if total_gt_area > 0 else 1.0
            total_precision = total_overlap_area / total_det_area if total_det_area > 0 else 1.0
            ...
            if total_missed_area > 0:
                print(f"Missed: {total_missed_area:,} px ({total_missed_area/total_gt_area:.1%} of faces NOT blurred)")
            print(f"{'='*60}")
